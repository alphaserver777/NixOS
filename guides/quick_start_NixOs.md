...
Если вы ставите систему из этого репозитория, ручной сценарий ниже можно не проходить. Вместо него используйте интерактивный скрипт:

```bash
sudo ./scripts/install-nixos.sh
```

Краткое описание и ограничения вынесены в [install_nixos_script.md](install_nixos_script.md).
