...
- Ответы агента преобразуются из Markdown в разметку Telegram; длинные ответы
  автоматически делятся на несколько сообщений.
- Роли подключаются прямо из `MARKETING/AGENT/skills/` рабочей копии проекта,
  поэтому правила команды не расходятся с репозиторием.
